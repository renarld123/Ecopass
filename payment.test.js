...
test('checkout requires a matching signed payment before issuing a paid pass', async t => {
  ...
  process.env.ADMIN_PASSWORD = 'local-payment-test-admin';
  process.env.SESSION_SECRET = 'local-payment-test-session-secret-long';
  ...
  const login = await post('/api/admin/login', { password: 'local-payment-test-admin' });
  const adminCookie = login.headers.get('set-cookie').split(';')[0];
  assert.equal((await fetch(`${base}/api/admin/registrations/${reference}/confirm-payment`, { method: 'POST', headers: { Cookie: adminCookie } })).status, 409);
  ...
  assert.equal((await readPass()).qrDataUrl, null);
  assert.equal((await fetch(`${base}/verify/${reference}`)).status, 403);
  ...
  assert.equal((await fetch(`${base}/verify/${reference}`)).status, 200);
  assert.equal((await post('/api/paymongo/webhook', event, signed(event))).status, 200);
