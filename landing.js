...
const registrationCounts={adult:0,foreign:0,senior:0,child:0};
...
function renderPass(pass){
  const group=Object.keys(pass.groups).filter(key=>pass.groups[key]>0).map(key=>`${pass.groups[key]} ${groupLabel(key)}`).join(' · ');
  const issued=pass.passIssued===true&&pass.paymentStatus==='PAID'&&Boolean(pass.qrDataUrl);
...
function openSecureCheckout(url){
  ...
  const width=Math.min(640,Math.max(420,window.screen.availWidth-40));
  const height=Math.min(780,Math.max(540,window.screen.availHeight-40));
  const left=Math.round(window.screenX+(window.outerWidth-width)/2);
  const top=Math.round(window.screenY+(window.outerHeight-height)/2);
  const popup=window.open(url||'about:blank','ecopassSecureCheckout',`popup=yes,width=${width},height=${height},left=${left},top=${top},scrollbars=yes,resizable=yes`);
...
async function refreshCheckoutPass(){
  ...
  if(pass.paymentStatus==='PAID'){
    sessionStorage.removeItem('ecopassPendingCheckout');
...
passModal?.querySelector('[data-registration-complete]')?.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(result.checkoutUrl){
      ...
      try{sessionStorage.setItem('ecopassPendingCheckout',JSON.stringify({id:checkoutPassId,url:checkoutUrl}))}catch{}
...
passModal?.querySelector('[data-download-pass]')?.addEventListener('click',()=>{if(currentPass?.passIssued&&currentPass.qrDataUrl)downloadVerifiedPass()});
...
async function showReturnedPayment(){
  ...
  if(window.opener&&!window.opener.closed){window.opener.postMessage({type:'ecopass-checkout-return',passId,result},location.origin);window.close();return}
  openPassModal();showRegistrationStep(3);
  const status=passModal.querySelector('[data-pass-status="3"]');
  const refresh=async()=>{
    const response=await fetch(`/api/passes/${encodeURIComponent(passId)}`,{cache:'no-store'});
    if(!response.ok)throw new Error('Your registration record could not be loaded.');
    const pass=await response.json();
    currentPass={...pass,name:pass.fullName,visitDateLabel:formatPassDate(pass.visitDate),validUntil:formatPassDate(pass.validUntil)};
    renderPass(currentPass);
    const paid=pass.passIssued===true&&pass.paymentStatus==='PAID';
    status.textContent=paid?'Payment confirmed. Your EcoPass QR is ready.':result==='cancel'?'Payment was cancelled. No QR pass has been issued.':'Payment is not confirmed yet. Your QR pass will appear only after confirmation.';
    if(paid)sessionStorage.removeItem('ecopassPendingCheckout');
    return paid;
  };
  try{if(!(await refresh())&&result==='return'){let tries=0;const timer=setInterval(async()=>{try{tries++;if(await refresh()||tries>=10)clearInterval(timer)}catch{clearInterval(timer)}},3000)}}catch(error){status.textContent=error.message}
  if(result==='cancel')sessionStorage.removeItem('ecopassPendingCheckout');
  history.replaceState({},'',location.pathname);
}
async function restorePendingCheckout(){
  if(new URLSearchParams(location.search).has('payment'))return;
  let pending;
  try{pending=JSON.parse(sessionStorage.getItem('ecopassPendingCheckout')||'null')}catch{return}
  if(!pending||!/^ECP-[0-9]{8}-[A-F0-9]{8}$/.test(pending.id)||!/^https:\/\/checkout\.paymongo\.com\//.test(pending.url))return;
  checkoutPassId=pending.id;checkoutUrl=pending.url;
  openPassModal();showRegistrationStep(3);
  const status=passModal.querySelector('[data-pass-status="3"]');
  try{
    const paid=await refreshCheckoutPass();
    if(!paid){checkoutSupport.hidden=false;status.textContent='Payment is still unconfirmed. No QR pass has been issued. Resume secure checkout or wait for confirmation.';watchSecureCheckout()}
  }catch(error){checkoutSupport.hidden=true;status.textContent=error.message}
}
